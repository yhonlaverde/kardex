...
export class VariedadComponent<t, k> implements OnInit {
  action: IAction<t, k>[];
  subessearch: any;
  parameterSub: any;
  searchSub: any;
  ...
  initvalue(): void {
    ...
    this.maestros.getVariedad(this.parameterSub, 'GET').subscribe((sub) => {
      this.subessearch = sub.variedadesItem;
      this.searchArticulo = sub.variedadesItem;
  ...
  SaveSub(): void {
    ...
    this.maestros.getVariedad(this.parameterSub, 'INS').subscribe((marc) => {
  ...
  EditSub(id): void {
    this.parameterSub = {
      idVariedadItem: this.id,
      codigo: this.code,
      descripcion: this.descripcion
    }

    this.maestros.getVariedad(this.parameterSub, 'UPD').subscribe((marc) => {
      $('#editMark').modal('hide')
      Swal.fire(
        'Good job!',
        'success',
  ...
  borrar(datos) {
    this.parameterSub = {
      idVariedadItem: datos.idVariedadItem,
      codigo: datos.codigo,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.maestros.getVariedad(this.parameterSub, 'DEL').subscribe((marc) => {
